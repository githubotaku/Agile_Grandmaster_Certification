...
export default function QuizPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6">
      <h1 className="text-2xl font-bold text-slate-900">1차 관문 · 지식 퀴즈</h1>
      <p className="mt-2 text-sm text-slate-600">
        아래 3문항을 모두 맞히면 1차 관문을 통과합니다.
      </p>

      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        {QUIZ_QUESTIONS.map((q, idx) => (
          <fieldset key={q.id} className="card">
            <legend className="mb-3 font-medium text-slate-900">
              {idx + 1}. {q.question}
            </legend>
        ...
        {passed ? (
          <div className="rounded-md bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            축하합니다! 1차 관문을 통과했습니다.{" "}
            <a href="/apply" className="font-semibold underline">
              2차 관문으로 이동하기
            </a>
          </div>
        ) : (
